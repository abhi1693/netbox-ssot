...
Collection produces immutable observations. Reconciliation produces a durable plan from selected observations and a target snapshot. Review records decisions on that plan. Apply is a separate capability that requires an approved, non-stale plan and a current target re-read.
