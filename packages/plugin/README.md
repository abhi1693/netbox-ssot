...
IPAM, DCIM, and Circuits resource, and the portable Users access-control graph through dependency-closed datasets: Data
...
circuits; circuit groups; IP registries, routing, VLANs, prefixes, addresses, FHRP, services; and cabling. Its automatic support graph also includes Tags, Owner Groups, Owners, Tenant
...
Config Templates and ASN Roles are first-class dependencies. Rack Reservation users must
...
generated files and synchronization/job state, Device primary-IP/cluster assignments, Interface wireless policy
fields, VM-interface assignments, wireless cable endpoints, contacts, and images are outside this provider-owned
boundary. Extras scripts, personal UI state, generated notifications/history, binary attachments, subscriptions, and
explicit webhook credentials and FHRP authentication keys are also excluded. Only the Git branch parameter is portable; destination-only Data
